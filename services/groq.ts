...
const getAvailableKeys = async (): Promise<string[]> => {
    ...
    // 1. Try Global Secure Keys from RTDB (Groq specific path if needed, or re-use secure_keys/list but we want separate keys)
    // For now, let's assume Groq keys are stored in SystemSettings primarily as requested.
    ...
    try {
        ...
        if (storedSettings) {
            ...
            if (parsed.groqApiKeys && Array.isArray(parsed.groqApiKeys)) {
                ...
                if (parsed.groqApiKeys.length > 0) console.log("Using Groq Keys from System Settings");
            }
        }
    } catch (e) {}
...
const callGroqApi = async (key: string, messages: any[], model: string = "llama3-8b-8192") => {
